...
import type { INavItem, TMenuView } from '../types';
import { viewIndex } from '../motion';
...
function useViewTransition<T extends INavItem>(items: T[]) {
  ...
  const setNextView = <T extends INavItem>(item: T) => {
    ...
    const nextDirection = view !== 'default' && nextView !== 'default' ? Math.sign(viewIndex(items, nextView) - viewIndex(items, view)) : 0;
