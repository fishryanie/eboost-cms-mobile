...
import Animated, { Extrapolation, interpolate, useAnimatedProps, useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';
import { DURATION, EASING, PANEL_SLIDE } from '../motion';

const AnimatedBlurView = Animated.createAnimatedComponent<typeof BlurView>(BlurView);

function usePanelMotion<T extends boolean, D extends number>(active: T, direction: D) {
  ...
  const androidBlurStyle = useAnimatedStyle<Pick<ViewStyle, 'filter'>>(() => ({
    filter: [
      {
        blur: withSpring(interpolate(progress.value, [0, 0.5, 1], [0, 10, 0], Extrapolation.CLAMP)),
      },
    ],
  }));
