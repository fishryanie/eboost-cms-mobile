...
export const AnimatedScrollView = forwardRef<ScrollView, AnimatedScrollViewProps>(
  (
    ...
    const [scroll, onScroll, scale, translateYDown, translateYUp] = useAnimateScrollView(imageHeight, disableScale, externalOnScroll);

    return (
      <>
        <Animated.ScrollView ref={ref} scrollEnabled={true} onScroll={onScroll} scrollEventThrottle={16} {...props}>
          <AnimatedHeader
            ...
            scale={scale}
            OverlayHeaderContent={renderOveralComponent ? renderOveralComponent!() : OverlayHeaderContent}
          />
          {children}
        </Animated.ScrollView>
        <AnimatedNavbar
          ...
          scroll={scroll}
          imageHeight={imageHeight}
          OverflowHeaderComponent={renderHeaderNavBarComponent ? renderHeaderNavBarComponent!() : HeaderNavbarComponent}
          TopNavbarComponent={renderTopNavBarComponent ? renderTopNavBarComponent() : TopNavBarComponent}
        />
      </>
    );
  },
