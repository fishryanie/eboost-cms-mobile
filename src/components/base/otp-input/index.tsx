import React, { createContext, memo, useContext, useEffect, useRef, useState, type FC, type FunctionComponent } from 'react';
import { Keyboard, Pressable, StyleSheet, TextInput, View, type ViewStyle } from 'react-native';
import Animated, {
  FadeInDown,
  FadeOutDown,
  LinearTransition,
...
const OtpItem: FC<IOtpItem> & FunctionComponent<IOtpItem> = ({ index }: IOtpItem): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => {
  const {
    inputRef,
    onPress,
    otpValue,
  ...
  const animatedInputStyle = useAnimatedStyle<Pick<ViewStyle, 'borderWidth' | 'borderColor' | 'backgroundColor' | 'transform'>>(() => {
    const normalBg = interpolateColor(focusProgress.value, [0, 1], [unfocusedBackgroundColor, focusedBackgroundColor]);

    const backgroundColor = interpolateColor(errorProgress.value, [0, 1], [normalBg, errorBackgroundColor]);

    const normalBorder = interpolateColor(focusProgress.value, [0, 1], [unfocusedBorderColor, focusedBorderColor]);

    const borderColor = interpolateColor(errorProgress.value, [0, 1], [normalBorder, errorBorderColor]);
    ...
    <View key={index} style={[containerStyle]}>
      ...
      <Pressable disabled={!editable} onPress={onPress} style={styles.overlay}>
        <Animated.View
          ...
          {otpValue[index] !== '' && (
            <Animated.Text entering={animations.entering} exiting={animations.exiting} style={getTextStyle()}>
              {otpValue[index]}
            </Animated.Text>
          )}
        </Animated.View>
...
export const OtpInput: FC<IOtpInput> & FunctionComponent<IOtpInput> = memo<IOtpInput>(
  ({
    ...
    inputRef.current = data.map((_, index) => (inputRef.current[index] = React.createRef<TextInput>()));
    ...
    const triggerCompleteAnimation = () => {
      opacity.value = withSequence<number>(withTiming(0.6, { duration: 900 }), withTiming(1, { duration: 900 }));
    };
    ...
    return (
      <OtpContext.Provider value={inputProps}>
        <Animated.View style={[styles.container]}>
          ...
          {error && errorMessage && (
            <Animated.Text entering={FadeInDown.duration(200)} exiting={FadeOutDown.duration(200)} style={styles.errorMessage}>
              {errorMessage}
            </Animated.Text>
          )}
        </Animated.View>
