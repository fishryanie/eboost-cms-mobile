...
export function OperationScreen() {
  ...
  const tileWidth = Math.min(serviceTileSize, Math.floor((width - screenHorizontalPadding * 2 - Spacing.three * 3) / 4));
  const isRefreshing =
...
function UserPickerSheet({
  ...
  return (
    <BottomSheetModal
      ...
      ref={ref}
      snapPoints={['72%', '88%']}>
      <ThemedView style={[styles.sheetHeader, { paddingBottom: Spacing.three }]}>
      ...
      <ThemedView paddingHorizontal={Spacing.four} paddingBottom={Spacing.three}>
      ...
      <BottomSheetFlatList
        contentContainerStyle={[styles.sheetList, { paddingBottom: bottom + Spacing.five }]}
        data={users}
        keyExtractor={user => String(user.id)}
        ListEmptyComponent={
          usersQuery.isLoading ? (
            <ThemedView gap={Spacing.three} paddingTop={Spacing.six}>
...
function ResultStep({ loading, onDone, result }: { loading: boolean; onDone: () => void; result: ResultState | null }) {
  ...
  return (
    <ThemedView gap={Spacing.four} style={styles.resultCard}>
      {loading ? (
...
function SegmentedInput({
  ...
  return (
    <ThemedView flexDirection='row' gap={Spacing.two} style={styles.segmented}>
      {options.map(option => {
...
function AtRiskSubscriptionSection({
  ...
  return (
    <ThemedView style={styles.analyticsSection}>
      ...
      <ThemedView flexDirection='row' gap={Spacing.three} style={styles.atRiskMetrics}>
...
function UserGrowthSection({ growth, summary }: { growth: UserGrowthChartItem[]; summary?: UserGrowthSummary }) {
  ...
  return (
    <ThemedView style={styles.analyticsSection}>
      ...
      <ThemedView flexDirection='row' gap={Spacing.three} style={styles.growthSummaryLine}>
        <GrowthMetric
      ...
      {chartItems.length ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.growthChartScroller}>
          {chartItems.map(item => {
            ...
            return (
              <ThemedView alignItems='center' gap={Spacing.one} key={item.time} style={styles.growthBarColumn}>
